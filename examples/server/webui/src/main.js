...
const models = {
  mistralAI: {
    ...
    baseUrl: BASE_URL,
    modelName: 'bartowski_Mistral-Small-24B-Instruct-2501-GGUF_Mistral-Small-24B-Instruct-2501-Q4_K_M',
  ...
  methods: {
    ...
    async generateMessage(currConvId) {
      ...
      try {
        ...
        let messages = [{ role: 'system', content: config.systemMessage }, ...normalizeMsgsForAPI(this.messages)];
        ...
        const paramsDraft = {
          ...
          model: config.currentModel.modelName,
        };
        let params = {
          ..._.pick(paramsDraft, config.currentModel.withParams),
          ...(_.includes(config.currentModel.withParams, 'custom') && config.custom.length
            ? JSON.parse(config.custom)
            : {}),
        };
        if (config.currentModel.hasOwnProperty('action')) {
          params = config.currentModel.action(params);
        }

        const chunks = sendSSEPostRequest(`${config.currentModel.baseUrl}/v1/chat/completions`, {
          method: 'POST',
          headers: {
...
function filterThoughtFromMsgs(messages) {
  return messages.map((msg) => {
    return {
      role: msg.role,
      content: msg.role === 'assistant' ? msg.content.split('</think>').at(-1).trim() : msg.content,
    };
  });
}
